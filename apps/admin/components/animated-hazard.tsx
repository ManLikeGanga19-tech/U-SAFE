...
// Moving green/black caution-tape stripe (same technique as the storefront).
// Scrolls the 45° hazard gradient by its exact horizontal repeat distance and
// wraps there, so the loop is perfectly seamless with zero DOM.
const PERIOD_X = 28 / Math.cos(Math.PI / 4); // ≈ 39.598px

export function AnimatedHazard({
  className = "h-3",
  speed = 44, // px per second
  bgClass = "bg-hazard-signal", // green/black
  reverse = false,
}: {
  className?: string;
  speed?: number;
